...
export function AdminTeamScreen() {
  ...
  async function handleInvite() {
    ...
    if (!name.trim() || !email.trim() || permissions.length === 0) {
      setFormError('Name, email, and at least one permission are required.');
      return;
    }
    setSubmitting(true);
    try {
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.page }]} edges={['top']}>
      <View style={styles.header}>
        <Text style={[styles.title, { color: theme.text }]}>Team & Permissions</Text>
        <Button title="+ Invite" onPress={openInvite} style={styles.inviteButton} />
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={[styles.empty, { color: theme.textMuted }]}>No team members yet.</Text>}
          renderItem={({ item }) => (
              ...
              <View style={styles.cardHeader}>
                <View style={styles.cardHeaderText}>
                  <View style={styles.nameRow}>
                    ...
                    {item.status === 'PENDING' ? <Badge label="Pending" tone="warning" /> : null}
                  </View>
      ...
      <Modal visible={inviting} transparent animationType="fade" onRequestClose={() => setInviting(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { backgroundColor: theme.surface }]}>
            <Text style={[styles.modalTitle, { color: theme.text }]}>Invite team member</Text>
            {formError ? <Text style={[styles.errorText, { color: theme.error }]}>{formError}</Text> : null}
            <TextField label="Name" value={name} onChangeText={setName} style={styles.fieldSpacing} />
            <TextField
              label="Email"
              value={email}
            ...
            />
            <Text style={[styles.permissionsLabel, { color: theme.text }]}>Permissions</Text>
            ...
            <View style={styles.actionsRow}>
              <Button title="Cancel" variant="ghost" onPress={() => setInviting(false)} style={styles.actionButtonFlex} />
              <Button title="Send invite" onPress={handleInvite} loading={submitting} style={styles.actionButtonFlex} />
            </View>
          </View>
        </View>
      </Modal>
      ...
      <Modal visible={!!removeTarget} transparent animationType="fade" onRequestClose={() => setRemoveTarget(null)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { backgroundColor: theme.surface }]}>
            <Text style={[styles.modalTitle, { color: theme.text }]}>Remove this team member?</Text>
            <Text style={[styles.confirmBody, { color: theme.textSecondary }]}>
              This revokes {removeTarget?.name}&apos;s admin access immediately.
            </Text>
            <View style={styles.actionsRow}>
              <Button title="Cancel" variant="ghost" onPress={() => setRemoveTarget(null)} style={styles.actionButtonFlex} />
              <Button
                title="Remove"
