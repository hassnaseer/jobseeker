...
function counterpartLabel(conversation: Conversation, userId?: string): string {
  const isClientSide = userId === conversation.clientId;
  const otherId = isClientSide ? conversation.seekerId : conversation.clientId;
  const otherLabel = isClientSide ? 'Freelancer' : 'Client';
        ...
        renderItem={({ item }) => (
          ...
          >
            <View style={styles.rowText}>
              <Text style={[styles.rowTitle, { color: theme.text }]}>{counterpartLabel(item, user?.id)}</Text>
              {item.contractId ? <Text style={[styles.rowMeta, { color: theme.textMuted }]}>Workroom</Text> : null}
            </View>
